...
derivations to the corresponding set of grafts."
  ;;; DESIGN DECISION: Always use canonical full list of outputs
  ;;;
  ;;; This fix addresses a critical bug where cumulative-grafts would produce
  ;;; different results based on the requested outputs, creating conflicting
  ;;; grafted versions of the same package in a single build plan.
  ;;;
  ;;; Advantages:
  ;;; - Ensures deterministic results: same derivation always produces same graft
  ;;; - Fixes runtime crashes from conflicting library grafts
  ;;; - Maintains derivation atomicity (multi-output packages stay atomic)
  ;;;
  ;;; Disadvantages:
  ;;; - Performance overhead: analyzes more dependencies than strictly needed
  ;;; - Storage overhead: builds unused outputs, causing larger substitute downloads
  ;;;   and temporary local storage bloat (mitigated by GC reclaiming unreferenced outputs)
  ;;; - Infrastructure burden: increased build farm and substitute server costs
  ;;; - API change: #:outputs parameter removed (always uses all outputs)

  ;; Always use the canonical full list of outputs to ensure deterministic
  ;; results and avoid conflicts from partial grafting.
  ;;
  ;; This ensures:
  ;; 1. Determinism: For any given derivation, only one canonical "grafted"
  ;;    version is ever computed and cached.
  ;; 2. Correctness: Enforces that multi-output derivations are atomic units.
  ;;    Any replacement must be a complete, multi-output derivation.
