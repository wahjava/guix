...
(define-configuration/no-serialization sane-configuration
  (sane
   ;; Using the fixed variant because it is the only one that contains native search paths.
   ;; TODO: Replace with "sane" when removing the fixed variant.
   (package sane/fixed)
   "A package that provides the SANE library.")
  (backends
   (non-empty-list-of-packages (list sane-backends))
   "A list of packages containing SANE backends."))

(define (sane-search-paths cfg)
  (match-record cfg <sane-configuration> (sane backends)
...
(define* (lift-sane-configuration cfg #:key warn?)
  (if (sane-configuration? cfg) cfg
      (begin
        (if warn?
            (warning (G_ "'sane' service now expects a 'sane-configuration' record~%")))
        (sane-configuration (backends (list cfg))))))
