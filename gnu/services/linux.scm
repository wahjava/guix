...
;;;
;;; Make a source path available at some other target path, with access rights
;;; according to one of three policies:
;;;
;;; - 'bind: ownership and permissions stay the same, this is a bind mount,
;;; it just gives another name to the original file
;;;
;;; - 'translate: the target acts as though it was owned by the specified
;;; target user, and writes happen on the source as if done by the source's
;;; owner. Note that chown or chgrp on the target will fail. This is
;;; implemented with bindfs.
;;;
;;; - 'overlay: the target acts as though it was owned by the specified
;;; target user, but writes are diverted to a target-user-owned directory and
...
(define-record-type* <vfs-mapping>
   ...
   (policy        vfs-mapping-policy       (default 'translate))
   (user          vfs-mapping-user         (default #f))
   (group         vfs-mapping-group        (default "users"))
   (name          vfs-mapping-name         (default (string-append
                                                          (vfs-mapping-source      this-record) "-["
                                                          (vfs-mapping-policy      this-record) "]->"
                                                          (vfs-mapping-destination this-record)))
                  (thunked))
   (requirement   vfs-mapping-requirement (default '(file-systems user-homes))))
                           ...
                           (let ((overlay (string-append #$destination "-overlay"))
                             ...
                             (mount "overlay"       ;; source
                                    ...
                                    (string-append  ;; options
                                     ...
                                     "upperdir=" overlay  ","
                                     "workdir="  workdir))
                             ;; remount the target over itself to make it
                             ;; appear as if owned by user-name and user-group
                             (invoke
                              #$bindfs
                              (string-append "--create-for-group="  source-group)
                              (string-append "--create-for-user=" source-user)
...
(define vfs-mapping-service-type
  (service-type
   ...
   (extend (lambda (original extensions)
             (vfs-mapping-configuration
              (inherit original)
              (bindings (append (vfs-mapping-configuration-bindings original) extensions)))))
   (default-value (vfs-mapping-configuration))
   (description "Share or expose a file or directory on another path")))
