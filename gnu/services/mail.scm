...
(define (getmail-serialize-field field-name val)
  #~(let ((val '#$val))
      (format #f "~a = ~a\n"
...
(define (getmail-serialize-string field-name val)
  (if (string=? val "")
      ""
      (serialize-field field-name val)))
...
(define (serialize-string-or-filelike field-name val)
  (if (equal? val "")
      ""
      (getmail-serialize-field field-name val)))

(define (getmail-serialize-boolean field-name val)
  (getmail-serialize-field field-name (if val "true" "false")))

(define serialize-list getmail-serialize-field)
...
(define (serialize-parameter-alist field-name val)
  #~(string-append
     #$@(map (match-lambda
               ((key . value)
                (getmail-serialize-field key value)))
             val)))
