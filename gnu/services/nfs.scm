...
(define conf-integer? integer?)

(define (serialize-conf-integer name value)
  (format #f "~a = ~a" (scheme->autofs name) value))

(define conf-boolean? boolean?)

(define (serialize-conf-boolean name value)
  (format #f "~a = ~a" (scheme->autofs name) (if value "yes" "no")))

(define (conf-log-level? x)
  (and (keyword? x)
       (member x '(#:none #:verbose #:debug))))

(define (serialize-conf-log-level name value)
       ...
       #~(begin
           ...
           (call-with-output-file (string-append #$output "/autofs.conf")
             (lambda (autofs-conf)
               ...
               (for-each
                (lambda (line)
                  (display line autofs-conf)
                  (display "\n" autofs-conf))
                (cons*
                 "[ autofs ]"
                 (string-append "master_map_name = " #$output "/auto.master")
                 '#$autofs-conf)))))))))

(define (autofs-activation config)
  (let* ((config-dir (autofs-build-config config))
         (autofs-conf (file-append config-dir "/autofs.conf"))
         (num-mounts (length (autofs-configuration-mounts config)))
         (direct-mount-points
          (map
    ...
    #~(begin
        ...
        (false-if-execption (delete-file #$autofs-conf))
        (unless (zero? #$num-mounts)
          (symlink #$autofs-conf "/etc/autofs.conf")))))
