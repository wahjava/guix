    ...
    (package
      ...
      (arguments
       (substitute-keyword-arguments (package-arguments base-rust)
         ((#:phases phases)
          `(modify-phases ,phases
             ;; TODO: Move this phase earlier on an eventual world rebuild,
             ...
             (add-after 'install 'post-install
               (lambda* (#:key outputs #:allow-other-keys)
                 ...
                 ;; around Rust 1.89.
                 (let ((rustlib (string-append (assoc-ref outputs "out")
                                               "/lib/rustlib")))
                   (for-each delete-file
                             (find-files rustlib
                                         "^librustc_driver.*\\.so$"))))))))))))
         ...
         ((#:phases phases)
          `(modify-phases ,phases
             (replace 'build
               ...
               ;; TODO: Try reverting when Rust 1.91 comes out, along with
               ;; using "./x.py install rustc std cargo" (which should be
               ;; fixed to not rebuild things unnecessarily). If the install
               ;; target works, the 'post-install' phase should no longer be
               ;; necessary.
               (lambda* (#:key parallel-build? #:allow-other-keys)
                 (let ((job-spec (string-append
                                   "-j" (if parallel-build?
...
exec -a \"$0\" \"~a\" \"$@\""
      ...
      (native-inputs (cons*
                      ;; Keep in sync with the llvm used to build rust.
                      `("clang-source" ,(package-source clang-runtime-20))
                      ;; Add test inputs.
                      `("gdb" ,gdb/pinned)
                      `("procps" ,procps)
                      (package-native-inputs base-rust)))
