    ...
    (package
      ...
      (arguments
       (list
        #:qtbase qtbase
        #:tests? #f                     ;no tests
        #:configure-flags #~(list "-DFETCHCONTENT_FULLY_DISCONNECTED=ON"
                          ...
                          (let ((dep (assoc-ref inputs "single-application")))
                            (substitute* "CMakeLists.txt"
                              (("^if\\(USE_SINGLEAPPLICATION\\)" all)
                               (string-append
                                all
                                "\nadd_library(SingleApplication::SingleApplication SHARED IMPORTED)"
                                "\nset_target_properties(SingleApplication::SingleApplication PROPERTIES"
