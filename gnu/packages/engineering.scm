  ...
  (package
    ...
    (source
     (origin
       ...
       (file-name (git-file-name name version))
       (patches (search-patches
                 "prusa-slicer-2.9.2-Add-CMake-module-for-finding-HidAPI-library.patch"))
       (sha256
       ...
       (snippet `(begin
                   ...
                   ;; (delete-file-recursively "bundled_deps/eigen")
                   ...
                   (substitute* "src/slic3r/CMakeLists.txt"
                     (("add_library\\(libslic3r_gui.*" all)
                      (string-append "find_package(HidAPI REQUIRED)\n" all
                       "
target_include_directories(libslic3r_gui PRIVATE ${HIDAPI_INCLUDE_DIR})
"))
                     (("    hidapi")
                      "    ${HIDAPI_LIBRARY}"))))))
    (build-system cmake-build-system)
    (arguments
     (list
      ...
      #~(list "-DSLIC3R_FHS=1" ;Use The Filesystem Hierarchy Standard.
              ...
              ;; Use wxWidgets 3.0.x.x to prevent GUI crashes when adding
              ;; support enforcers.
              "-DSLIC3R_WX_STABLE=1"
    ...
    (inputs (list boost
                  ...
                  pango
                  prusa-libbgcode
                  ;; XXX: Using Prusa wxWidgets fork as PrusaSlicer segfaults
                  ;; when compiled with regular wxwidgets.
                  prusa-wxwidgets
                  qhull
                  tbb
                  webkitgtk-for-gtk3
...
G-code instructions for FFF printers or PNG layers for mSLA 3D printers.  It's
